fitPP.fun <-function(covariates=NULL,start, fixed = list(),
  ...
  if(is.null(tim)) tim <- c(1:nobs)

  Bcovariates <- as.matrix(covariates[inddat==1, ])
  covariatest <- as.matrix(covariates[posE, ])

  # This is the reason that only 50 covariates are allowed.
  minuslogl <- function(
    b0=NULL,b1=NULL,b2=NULL,b3=NULL,b4=NULL,b5=NULL,b6=NULL,
  ...
  if (dplot)
  {
    if((modCI) & (is.null(lambdaylim))) {
      lambdaylim <- c(min(LIlambda, na.rm = TRUE), max(UIlambda, na.rm = TRUE))
    }
    ...
    if (modCI) {
      ...
      legend(xlegend,
             legend=c('Fitted intensity',
                      paste('Upper CI',CIty,sep=' '),
                      paste('Lower CI', CIty, sep=' ')),
  ...
  if(tind)	{
    covariates <- checkdim(covariates, j = 1)
  }
  ...
  new('mlePP', call = call, coef = coef, fullcoef = fullcoef,
      ...
      method = minfun, npar = npar,inddat = inddat,
      lambdafit = lambdafit,  LIlambda = LIlambda, UIlambda = UIlambda,
